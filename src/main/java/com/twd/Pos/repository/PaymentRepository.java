...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    //         +
    //         "FROM Payment p LEFT JOIN p.order o WHERE p.isSuccessful = true")
    // List<PaymentOrderDTO> findPaymentsWithOrderDetails();

    @Query("SELECT new com.twd.Pos.dto.PaymentOrderDTO(p.id, p.paymentMethod, p.amountPaid, p.paymentDate, o.customOrderId, o.total, m.id) " +
       "FROM Payment p " +
       "LEFT JOIN p.order o " +
